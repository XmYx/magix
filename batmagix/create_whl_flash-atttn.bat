@echo off
::-----------------------------------------------------------------
::  FlashAttention Windows Wheel Wizard ✨ (CUDA 12.8 + Torch nightly)
::-----------------------------------------------------------------
::  Works on Windows 10/11 x64, VS 2022 BuildTools 17.4+, CUDA 12.8.x
::  Needs: git, Python ≥ 3.10 on PATH, NVIDIA driver ≥ 555.
::-----------------------------------------------------------------

:: =================== USER-TWEAKABLE SETTINGS =====================
set "FA_COMMIT=98edb0d1a7d4e5106b1e555823b899c9e1f5f4de"  :: repo revision / tag
set "PY_VER=3.10"                                         :: Python ABI tag
set "CUDA_TAG=cu128"                                      :: CUDA wheels suffix
set "ENVNAME=fa_build_%PY_VER%"                           :: virtual-env dir
set "MAX_JOBS=4"                                          :: limit clang/nvcc RAM use
:: ===============================================================

SETLOCAL ENABLEDELAYEDEXPANSION
chcp 65001  >nul 2>&1   :: UTF-8 for emoji output

:: ---------- helper to print coloured lines ----------------------
:cecho
::  %1 = colour name, rest = message
setlocal
set "_c=%~1"
shift
powershell -NoProfile -Command "param([string]$c,[string]$m); Write-Host $m -ForegroundColor $c" "%_c%" "%*"
endlocal & exit /b
...
call :info "[4/6] Configuring MSVC BuildTools…"
...
python - <<PY
import flash_attn, sys; print('🪄 FlashAttention import OK – wheel looks good!')
PY
if errorlevel 1 (call :err "import test failed")
